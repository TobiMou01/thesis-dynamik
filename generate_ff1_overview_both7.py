#!/usr/bin/env python3
"""
FF1 Big-Picture Jointplot — VARIANTE mit 7 Boxplots pro Kennzahl auf BEIDEN Marginalen.

Identisch zur Original-Grafik (Notebook kap4_FF1.ipynb, Zelle 3), aber sowohl der
obere phi1-Marginal als auch der rechte sigma(dY)-Marginal zeigen jetzt je 7
Boxplots — einen pro Kennzahl (nach Schicht eingefaerbt) — statt 3 Schicht-Boxplots.
Der Haupt-Scatter (3 Schichten) bleibt unveraendert.

Erzeugt eine NEUE Datei, die anderen Varianten bleiben unangetastet.
"""
...
OUT_PNG = ROOT / "output" / "plots" / "ff1_overview_jointplot_log_beide_pro_kennzahl.png"
...
# ── Oberer Marginal-Boxplot — NEU: 7 Kennzahlen statt 3 Schichten (phi1) ─
top_data = [ar_bp[ar_bp["ratio"] == r]["phi1"].values for r in RATIO_ORDER]
box_colors = [LAYER_COLORS[RATIO_LAYER[r]] for r in RATIO_ORDER]
bp_top = ax_top.boxplot(top_data, vert=False, widths=0.6, patch_artist=True,
                        positions=range(len(RATIO_ORDER)), showfliers=False,
                        medianprops=dict(color="black", linewidth=1.4))
for patch, c in zip(bp_top["boxes"], box_colors):
    patch.set_facecolor(c); patch.set_alpha(0.7)
ax_top.set_yticks(range(len(RATIO_ORDER)))
ax_top.set_yticklabels([RATIO_SHORT[r] for r in RATIO_ORDER], fontsize=8)
ax_top.tick_params(axis="x", labelbottom=False)
ax_top.spines["bottom"].set_visible(False)
ax_top.set_xlim(phi_lo, phi_hi)
ax_top.axvline(phi1_med_bp, color="#444", linewidth=0.9, linestyle="--", alpha=0.7)

# ── Rechter Marginal-Boxplot — 7 Kennzahlen (sigma) ──────────────────────
...
bp_right = ax_right.boxplot(right_data, vert=True, widths=0.6, patch_artist=True,
                            positions=range(len(RATIO_ORDER)), showfliers=False,
                            medianprops=dict(color="black", linewidth=1.4))
for patch, c in zip(bp_right["boxes"], box_colors):
    patch.set_facecolor(c); patch.set_alpha(0.7)
...
# ── Referenz: Median phi1 und sigma pro Kennzahl ─────────────────────────
print("\nMedian phi1 / sigma(dY) pro Kennzahl:")
for r in RATIO_ORDER:
    sub = ar_bp[ar_bp["ratio"] == r]
    print(f"  {RATIO_SHORT[r]:5s} ({RATIO_LAYER[r]:15s}) n={len(sub):3d}  "
          f"phi1_med={sub['phi1'].median():+.4f}  sigma_med={sub['sigma_diff'].median():.4f}")
